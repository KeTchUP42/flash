...
void Mobs::Mushroom::selfMove(Unite::Unifier *unifier) {
    ...
    if ((obstacle = m_algorithms->getCollision()->getMovingObstacleCollision().abscissaMoveAble(this, unifier)) != nullptr) {
        m_properties.speed.xSpeed = static_cast<int>(-1 * m_properties.speed.xSpeed);
    }

    if ((obstacle = m_algorithms->getCollision()->getMovingObstacleCollision().ordinateMoveAble(this, unifier)) != nullptr) {
    ...
    for (const std::shared_ptr<Mobs::Player> &player : unifier->getPlayers()) {
        Mobs::Monster *me;
        if ((me = m_algorithms->getCollision()->getMovingMonsterCollision().ordinateMoveAble(player.get(), unifier)) != nullptr) {
            if (me == this) {
    ...
    if ((monster = m_algorithms->getCollision()->getMovingMonsterCollision().abscissaMoveAble(this, unifier)) != nullptr) {
