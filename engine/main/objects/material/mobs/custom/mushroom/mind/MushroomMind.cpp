...
void Mobs::MushroomMind::analyze(Unite::Unifier *unifier) noexcept {
    ...
    Mobs::Mob *mob;
    if ((mob = m_holder->m_algorithms->getCollision().getMovingCollision().abscissaMoveAble(m_holder, unifier->getMobs())) != nullptr) {
        if (m_holder->getSpeed().xSpeed * mob->getSpeed().xSpeed <= 0) {
            mob->setSpeed(Components::Speed((m_holder->getSpeed().xSpeed > 0 ? 1 : -1) * m_holder->m_mushroom.punchPower,
                                                mob->getSpeed().ySpeed - m_holder->m_mushroom.punchPower * 2));
